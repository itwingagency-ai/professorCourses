...
import { useGetAllQuizzesAdminQuery, useArchiveQuizAdminMutation } from "@/redux/features/quiz/quizApi";
import toast from "react-hot-toast";
import { format } from "timeago.js";

const AllQuizzes = () => {
  const { theme } = useTheme();
  const { data, isLoading, refetch } = useGetAllQuizzesAdminQuery();
  const [archiveQuiz, { isSuccess, error }] = useArchiveQuizAdminMutation();

  useEffect(() => {
    if (isSuccess) {
      toast.success("Quiz archive status updated");
      refetch();
    }
    if (error) {
      if ("data" in error) {
        toast.error((error as any).data?.message || "Failed to update quiz");
      }
    }
  }, [isSuccess, error, refetch]);

  const handleArchive = async (id: string) => {
    if (window.confirm("Are you sure you want to change the archive status of this quiz?")) {
  ...
  const columns = [
    ...
    { field: "passingMarks", headerName: "Passing Marks", flex: 0.4 },
